...
class CoordinateProcessorParams:
    ...
    output_dir: str
    clustering_logs_dir: str
    initial_base: InitialBaseParams
...
def get_coordinate_receiver_params(node: Node) -> CoordinateReceiverParams:
    ...
    node.declare_parameter("motion_stability_threshold", 0.5)
    node.declare_parameter("processing_delay_estimate", 0.1)
    node.declare_parameter("base_frame_timeout", 0.2)
    node.declare_parameter("min_frame_timeout", 0.05)
    node.declare_parameter("max_frame_timeout", 0.3)
    node.declare_parameter("velocity_scale_factor", 0.1)
    node.declare_parameter("motion_confidence_factor", 1.0)
    ...
    node.declare_parameter("camera.bias_y", -0.00130)
    node.declare_parameter("camera.fx", 917.06)
    node.declare_parameter("camera.fy", 917.06)
    node.declare_parameter("camera.cx", 639.5)
    node.declare_parameter("camera.cy", 359.5)
    node.declare_parameter("camera.baseline", 0.052)
    node.declare_parameter("camera.rgb_width", 1920)
    node.declare_parameter("camera.rgb_height", 1080)
    node.declare_parameter("camera.depth_width", 1280)
    node.declare_parameter("camera.depth_height", 720)
    ...
    motion_params = MotionParams(
        ...
        stability_threshold=node.get_parameter("motion_stability_threshold").value,
        ...
        confidence_factor=node.get_parameter("motion_confidence_factor").value,
        velocity_outlier_threshold=node.get_parameter(
            "velocity_outlier_threshold"
        ).value,
        outlier_velocity_threshold=node.get_parameter(
            "outlier_velocity_threshold"
        ).value,
        outlier_distance_threshold=node.get_parameter(
...
def get_coordinate_processor_params(node: Node) -> CoordinateProcessorParams:
    ...
    node.declare_parameter("expected_bases", 5)
    node.declare_parameter("outlier_threshold", 1.5)
    node.declare_parameter("output_dir", "/root/ros2_ws/tuning_results/default_run")
    node.declare_parameter("clustering_logs_dir", "/root/ros2_ws/clustering_logs")
    ...
    return CoordinateProcessorParams(
        expected_bases=node.get_parameter("expected_bases").value,
        outlier_threshold=node.get_parameter("outlier_threshold").value,
        output_dir=node.get_parameter("output_dir").value,
        clustering_logs_dir=node.get_parameter("clustering_logs_dir").value,
        initial_base=initial_base_params,
        ground_truth_bases=ground_truth_bases,
